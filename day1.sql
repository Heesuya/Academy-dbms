...
--1. 입사일이 5년 이상, 10년 이하인 직원의 이름, 주민번호, 급여, 입사일을 이름순으로 조회
-- 입사한지 몇일 지났는지 계산하는 방법
-- 오라클에서 날짜1 - 날짜2 -> 날짜1 과 날짜2 사이의 일수가 숫자로 반환
-- 오라클에서 오늘날짜표현 -> sysdate -> 현재 시스템시간

select sysdate from dual;    --dual은 데이터를 확인하기위한 임시테이블 (테이블인척)
...
--2. 재직중이 아닌 직원의 이름, 부서코드, 입사일, 퇴사일을 검색
select * from employee;
-- 퇴사여부 확인 1) ENT_DATE(퇴사일)가 존재하는 경우, 2) ENT_YN 컬럼값이 Y인 경우
select
    emp_name, dept_code, hire_date,(ENT_DATE - hire_date), ent_date
from
    employee
where
    ent_yn = 'Y';
...
-- 문제 3
select 
    emp_name, SALARY+salary/2 인상급여, (sysdate-hire_date)/365 근속년수 
from 
    employee
where
...
-- 문제 4
select
    emp_name, emp_no, email, phone, salary
from 
    employee
where
